...
echo "Which browser should allow auto-open?"
echo "请选择运行脚本时自动打开的浏览器："
echo ""
echo "1. Google Chrome"
echo "2. Microsoft Edge"
echo "3. System Default (Safari/Other) / 系统默认"
echo ""
...
TARGET_FILE="$SCRIPT_DIR/run-os.command"
...
if [ -f "$TARGET_FILE" ]; then
    case "$browser_choice" in
        1)
            # 修改为 Chrome
            # 使用 sed 查找包含 open...http 的部分并替换
            sed -i '' 's/open.*"http:\/\/localhost:5173"/open -a "Google Chrome" "http:\/\/localhost:5173"/g' "$TARGET_FILE"
            echo "✅ Updated $TARGET_FILE to use Google Chrome."
            echo "已配置为使用 Google Chrome 启动。"
            ;;
        2)
            # 修改为 Edge
            sed -i '' 's/open.*"http:\/\/localhost:5173"/open -a "Microsoft Edge" "http:\/\/localhost:5173"/g' "$TARGET_FILE"
            echo "✅ Updated $TARGET_FILE to use Microsoft Edge."
            echo "已配置为使用 Microsoft Edge 启动。"
            ;;
        *)
            # 恢复默认 (去掉 -a 参数)
            sed -i '' 's/open.*"http:\/\/localhost:5173"/open "http:\/\/localhost:5173"/g' "$TARGET_FILE"
            echo "✅ Updated $TARGET_FILE to use system default browser."
            echo "已配置为使用系统默认浏览器启动。"
            ;;
    esac
else
    echo "⚠️ Warning: $TARGET_FILE not found. Skipping browser config."
    echo "警告：未找到运行脚本，跳过浏览器配置。"
fi

# ... 下面是 Initialization Complete ...